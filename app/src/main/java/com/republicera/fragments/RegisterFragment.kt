...
class RegisterFragment : Fragment(), GeneralMethods {
    ...
    var refListener: ListenerRegistration? = null
    var refListenerEmail: ListenerRegistration? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        privacyPolicy.setOnClickListener {
            activity?.startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://app.termly.io/document/privacy-policy/6fc84c8a-3fc0-43a5-ba02-243b7ad5100b")))
        }
...
//        Patterns.EMAIL_ADDRESS.matcher(logEmail).matches()  <--- this methos was used before for the if statement but I've replaced it as I kept getting the invalid email error
    ...
    private fun addUserToFirebaseDatabase(
        ...
        if (case == 0) {
            ...
            refListener = userRef.addSnapshotListener { documentSnapshot, firebaseFirestoreException ->
        ...
        if (uid != null) {
            FirebaseAuth.getInstance().currentUser!!.reload().addOnSuccessListener {
                ...
                if (updatedUser!!.isEmailVerified) {
                    val userRef = FirebaseFirestore.getInstance().collection("users").document(uid)
                    refListenerEmail = userRef.addSnapshotListener { documentSnapshot, firebaseFirestoreException ->
                        if (documentSnapshot != null && documentSnapshot.exists()) {
                            val intent = Intent(this.context, MainActivity::class.java)
                            intent.flags = Intent.FLAG_ACTIVITY_CLEAR_TASK.or(Intent.FLAG_ACTIVITY_NEW_TASK)
                            startActivity(intent)
